...
def train_model(config_path: str = "configs/config.yaml"):
    ...
    with mlflow.start_run(run_name=f"xgb_{run_ts}") as run:
        ...
        threshold, threshold_sweep = tune_threshold(val_probs, y_val, beta=beta)
        threshold_analysis = {"best_threshold": threshold, "sweep": threshold_sweep}
        
        threshold_analysis_path      = Path(cfg["artifacts"]["threshold_analysis"])
        with open(threshold_analysis_path , "w") as f:
            json.dump({"best_threshold": threshold, "sweep": threshold_analysis}, f, indent=2)
        ...
        classification_report_path      = Path(cfg["artifacts"]["classification_report"])
        with open(classification_report_path, "w") as f:
            json.dump(report, f, indent=2)
            
        confusion_matrix_path      = Path(cfg["artifacts"]["confusion_matrix"])
        ...
        test_metrics_path      = Path(cfg["artifacts"]["test_metrics"])
        ...
        model_path  = cfg["artifacts"]["model"]
